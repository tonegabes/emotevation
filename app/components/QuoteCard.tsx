...
const QuoteCard: React.FC<QuoteCardProps> = ({ name, date, quote, isUnmotivational = false }) => {
  return (
    <div className={`w-full max-w-xl p-8 mx-auto backdrop-blur-lg rounded-xl shadow-xl border transform transition-all hover:scale-[1.01] relative mt-10 ${
      isUnmotivational 
        ? 'bg-gray-800/10 border-gray-600/20' 
        : 'bg-white/10 border-white/20'
    }`}>
      <div className={`absolute -top-10 left-1/2 transform -translate-x-1/2 w-20 h-20 rounded-full flex items-center justify-center shadow-lg border-4 ${
        isUnmotivational
          ? 'bg-gradient-to-br from-gray-600 to-gray-800 border-gray-500/20'
      ...
      <div className="flex flex-col items-center">
        <h2 className={`text-2xl font-bold text-center mb-2 mt-10 bg-clip-text text-transparent ${
          isUnmotivational
